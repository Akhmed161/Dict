pattern = {
    ...
    'есим': ['esim(\w)?', 'е-сим', 'easim(\w)?', 'e сим', 'е сим', 'е -сим', 'e-sim(\w)?', 'esim-карт(\w)?', 'есим-карт(\w)?', 'esim карт(\w)?', 'есим карт(\w)?', 'виртуальн\w* sim', 'виртуальн\w* сим\w*', 'электрон\w* сим\w*', 'есимк(\w)?', 'вирт сим', 'esum(\w)?', 'к сим'],
    'нано': ['nano'],
