pattern = {
    ...
    'архтп': ['\b(мой|супер|везде) онлайн( плюс| ?\+)? ?(.)(\d+)(.)?(\d{2,4})\b', '\bинтернет для устройств( плюс| ?\+)? ?(.)(\d+)(.)?(\d{2,4})\b', '\bмой (разговор|раговор) ?(.)(\d+)(.)?(\d{2,4})\b', '\bмой (разговор|раговор|онлайн) ?(\d{2}) ?(\d{2}) ?(\d{2,4})\b'],
    'говно': ['гавно', 'дермо'],
    'мобильный': ['могильный'],
    'домашний': ['ljvfiybq'],
