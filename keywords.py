pattern = {
    ...
    'айти': ['it', 'айтишник(.)?', 'it админ', 'it-специалист(.)?', 'it admin', 'айти-специалист(.)?', 'ит-специалист(.)?'],
    'сапорт': ['support'],
    ...
    'е1 закупки': ['e1', 'e-1', 'e1-procurement', 'e1 procurement', 'е1'],
    ...
    #в основном общие
    'симка': ['sim-карт(.)?', 'сим карт\w*?', 'sim', 'симок', 'sim карт(.)?', 'сим-карт(.)?', 'сим', 'симкарт(.)?', 'simкарт(.)?', 'сим к(.)?'],
    'есим': ['esim', 'е-сим', 'е сим', 'e-sim', 'esim-карт(.)?', 'есим-карт(.)?', 'esim карт(.)?', 'есим карт(.)?', 'виртуальная sim', 'виртуальная сим', 'электронная сим', 'есимк(.)?'],
    'нано': ['nano'],
    'гигабайт': ['гб', 'gb', 'гиг(.)?', 'гигов', 'гигами', 'гбайт\w*', 'гибогайт(.)?', 'кигобайт(.)?', 'гигабвйт(.)?', 'гигобацт\w*', 'гигаб'],
    ...
    'пак': ['puk-код(.)?', 'puck', 'пук-код(.)?', 'puk код(.)?', 'puc код(.)?', 'пак-код(.)?', 'рuk код(.)?', 'паккод(.)?', 'пуккод(.)?', 'puk2', 'puk-2', 'пак2', 'пак-2', 'пак2 код', 'puk', 'рак код', 'пук код', 'пак(.)?'],
    'пин': ['pin-код(.)?', 'pin код(.)?', 'пинкод', 'пин-код(.)?', 'pin2', 'pin-2', 'пин2', 'пин-2', 'пин 2', 'pin'],
    'микс': ['mix(.)?', 'miks', 'михх'],
    'винк': ['wink(.)?', 'vink', 'wing'],
    ...
    'мнп': ['mnp'],
    'фрэндс': ['friends'],
    'теле2': ['tele-2', 'tele2', 'теле 2', 'tele 2', 'теледва', 'теле два', 'теле', 'т2 мобайл', 'т2-мобайл', 'телле 2', 'телле2'],
    ...
    'айпи': ['IP'],
    'впн': ['vpn', 'випиэн'],
    'тариф': ['ТП', 'тараф(.)?', 'тапиф', 'ториф', 'таррив(.)?', 'тарив(.)?', 'тариы', 'твриф', 'тартф', 'тприф', 'таариф', 'трариф', 'тармфы', 'тарий', 'тармф(.)?', 'таиифы'],
    'тарифы': ['тарифв'],
    'говно': ['гавно', 'дермо'],
    'интернет': ['инет\w*', 'internet', 'интырнкт', 'тньернет', 'инэт', 'инт-т', 'интеренет', 'интирнет', 'ин т', 'ин-т', 'интерне', 'иньернет(.)?', 'интернкт(.)?', 'интенет(.)?', 'иниернет(.)?', 'интнрнет(.)?', 'интернер'],
    ...
    'учетная запись': ['уз', 'учетк(.)?', 'учётк(.)?', 'уч(\.)? запис(.)?', 'уч(\.)?запис(.)?', 'учетка ad', 'учетка ад', 'ad', 'active directory', 'учётка ad', 'учётка ад'],
    ...
    'обещанный платеж': ['обещенны(.)? платеж(.)?', 'обеденны(.)? платеж(.)?', 'обешенны(.)? платеж(.)?', 'обещаны(.)? платеж(.)?', 'обнщаны(.)? платеж(.)?'],
    'другой': ['др.'],
    'исходящий': ['исх(\.)?'],
    'входящий': ['вх(\.)?'],
    ...
    'ошибочный': ['ошибный', 'ошиочнй', 'ошиочны(.)?'],
    'номер': ['елмер', 'номр'],
    ...
    'восстановление': ['восстанорвлени(.)?', 'востановлерни(.)?', 'воссатновлени(.)?', 'восстанволени(.)?', 'востановдлени(.)?', 'востоновлени(.)?', 'восстанволни(.)?', 'восставнолни(.)?', 'вовтановлени(.)?', 'воссатвнолени(.)?', 'Востанови', 'востановка', 'востанов', 'ВОСТАНОВИЧ'],
    'почините': ['прчините'],
    ...
    'абонентская': ['абанентская', 'обонентская', 'обанетская', 'абон(.)?', 'обоненская', 'обон'],
    'вип': ['VIP'],
    'селекшен': ['Selection'],
    'не': ['нп'],
    'звонить': ['заонить'],
    ...
    'блэк': ['блейк', 'Black'],
    'стоимость': ['Стомость',],
    'абонентская плата': ['АП', 'абон(\.)?плата', 'абон(.)? плат(.)?', 'абанентски рлата', 'анон плат(.)?'],
    'разблокируйте': ['Расблоктруйте', 'Black'],
    'сколько': ['скок(.)?', 'сколко'],
    'миа': ['мия', 'мии'],
    'подключить': ['потклучит(.)?', 'потключить', 'подклучит(.)?', 'падклучит(.)?'],
    'подключен': ['потключен'],
    'подключил': ['потключил'],
    'отключить': ['атклечить'],
    ...
    'почему': ['пачему', 'пасему', 'посему', 'пачиму', 'почиму', 'почемуттак', 'пояему', 'в связи с чем', 'связи с чем', 'с чем связан(.)?', 'с чем это связан(.)?', 'с чем это может быть связан(.)?', 'почечу', 'по чему', 'какого хрена бл'],
    'вы': ['ви'],
    'кино': ['кинчик'],
