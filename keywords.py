pattern = {
    ...
    'говно': ['гавно', 'дермо'],
    'интернет': ['инет\w*', 'internet', 'интрнет(\w)?', 'интырнкт', 'тньернет', 'инэт', 'инт-т', 'интеренет', 'интернэт(\w)?', 'интирнет', 'ин т', 'ин-т', 'интерне', 'иньернет(\w)?', 'иньрентет(\w)?', 'интернкт(\w)?', 'интенет(\w)?', 'иниернет(\w)?', 'интнрнет(\w)?', 'интернер', 'энтернет', 'интерет(\w)?', 'ентернет(\w)?', 'интетнет(\w)?', 'инернет(\w)?', 'интэрнэт(\w)?', 'интрнт', 'энторнет(\w)?', 'онтернет(\w)?', 'интеинтернет(\w)?'],
    'трафик': ['трвфмк(\w)?'],
